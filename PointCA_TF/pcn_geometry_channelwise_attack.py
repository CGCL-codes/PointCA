...
from utils.pcn_tf_util import chamfer, chamfer_partial_inputs, earth_mover, pairwise_distance, knn, get_pc_neighbors, channelwise_project_constraint
...
def attack(args):
    ...
    for c in range(len(pc_classes)):    # attack for each class     len(pc_classes)
        ...
        for b in range(n_batches):   # generating adv_examples by # of batchs    # n_batches
            ...
            for iteration in range(args.num_iterations):    # args.num_iterations
                ...
                delta = channelwise_project_constraint(adv_point_cloud - p_source_pc, -args.epsilon, args.epsilon, p_source_pc)
    ...
    df.to_csv(os.path.join(save_dir, 'statistic_table_geometry_channelwise_epsilon%s.csv' % str(args.epsilon)), encoding="utf-8-sig", mode="a", header=True, index=False)
