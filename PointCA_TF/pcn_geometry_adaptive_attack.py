...
save_dir = os.path.join(output_path, 'geometry_adaptive_attack_%s' % str(time_stamp))
os.makedirs(os.path.join(save_dir), exist_ok=True)
h5_dir = create_dir(os.path.join(save_dir, 'adv_example'))


# Output log file
log_file = os.path.join(save_dir, 'log_geometry_adaptive_eta%.3f.txt' % args.eta)
...
def attack(args):
    ...
    for c in range(len(pc_classes)):    # attack for each class     len(pc_classes)
        ...
        for b in range(n_batches):   # generating adv_examples by # of batchs    # n_batches
            ...
            feed_dicts = {inputs: p_source_pc, npts: npts_batch, gt: source_gt_batch}
    ...
    df.to_csv(os.path.join(save_dir, 'statistic_table_geometry_adaptive_eta%s_knn%s.csv' % (str(args.eta), str(args.knn))), encoding="utf-8-sig", mode="a", header=True, index=False)
