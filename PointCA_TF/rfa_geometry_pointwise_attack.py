...
from utils.rfa_tf_util import chamfer_per_pc, earth_mover, pointwise_project_constraint
...
def attack(args):
    ...
    for c in range(len(pc_classes)):    # attack for each class     len(pc_classes)
        ...
        for b in range(n_batches):   # generating adv_examples by # of batchs    # n_batches
            ...
            for iteration in range(args.num_iterations):    # args.num_iterations
                ...
                delta = pointwise_project_constraint(adv_point_cloud, args.radius, p_source_pc)
    ...
    df.to_csv(os.path.join(save_dir, 'statistic_table_geometry_pointwise_radius%.3f.csv' %args.radius), encoding="utf-8-sig", mode="a", header=True, index=False)
