...
from utils.rfa_tf_util import chamfer_per_pc, earth_mover, pairwise_distance, knn, get_pc_neighbors, local_geometric_density, adaptive_project_constraint
...
parser.add_argument('--model_type', default='rfa_layers')
...
parser.add_argument('--base_lr', type=float, default=0.75)
parser.add_argument('--decay_rate', type=float, default=0.6)
parser.add_argument('--Lambda', type=float, default=20)
...
def attack(args):
    ...
    inputs = tf.placeholder(tf.float32, (BATCH_SIZE, NUM_POINT, 3))
    inputs_adv = tf.placeholder(tf.float32, (BATCH_SIZE, NUM_POINT, 3), 'inputs_adv')
    ...
    adj_matrix = pairwise_distance(inputs_adv)
    nn_idx = knn(adj_matrix, k=args.knn)
    point_cloud_neighbors = get_pc_neighbors(inputs_adv, nn_idx=nn_idx)
    ...
    model = model_module.Model(inputs, gt, tf.constant(1.0), is_training=False)
    model_adv = model_module.Model(adv_pc, gt, tf.constant(1.0), is_training=False)

    latent_target_vector = model.gl_feature
    latent_adv_vector = model_adv.gl_feature
    ...
    loss_latent_kl = tfp.distributions.kl_divergence(P, Q)   # target source

    output_adv = model_adv.outputs
    cd_op, cd_per_pc = chamfer_per_pc(output_adv, gt)
    emd_op = earth_mover(output_adv, gt)

    loss_dist, dist_per_pc = chamfer_per_pc(inputs_adv, adv_pc)
    ...
    for c in range(len(pc_classes)):    # attack for each class     len(pc_classes)
        ...
        for b in range(n_batches):   # generating adv_examples by # of batchs    # n_batches
            ...
            feed_dicts = {inputs_adv: p_target_pc, gt: target_pc_batch}
            ...
            feed_dicts = {inputs_adv: p_source_pc, gt: source_gt_batch}
            ...
            for iteration in range(args.num_iterations):    # args.num_iterations
                feed_dicts = {inputs_adv: p_source_pc, inputs: p_target_pc, gt: target_pc_batch}
