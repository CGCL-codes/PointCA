...
from utils.pcn_tf_util import chamfer, chamfer_partial_inputs, earth_mover, pairwise_distance, knn, get_pc_neighbors, pointwise_project_constraint
...
save_dir = os.path.join(output_path, 'geometry_pointwise_attack_%s' % str(time_stamp))
os.makedirs(os.path.join(save_dir), exist_ok=True)
h5_dir = create_dir(os.path.join(save_dir, 'adv_example'))


# Output log file
log_file = os.path.join(save_dir, 'log_geometry_pointwise_radius%.3f.txt' % args.radius)
...
def attack(args):
    ...
    pc_parts = tf.squeeze(tf.split(inputs, BATCH_SIZE, axis=1))
    ...
    model = model_module.Model(adv_pc, npts, gt, tf.constant(1.0), is_training=False)

    output = model.outputs
    cd_op, cd_per_pc = chamfer(output, gt)
    emd_op = earth_mover(output, gt)

    loss_dist, dist_per_pc = chamfer_partial_inputs(inputs, adv_pc, BATCH_SIZE)
    loss = cd_op

    lr = tf.train.exponential_decay(learning_rate=args.base_lr, global_step=args.num_iterations, decay_steps=20, decay_rate=0.7, staircase=True, name="learning_rate_decay")
    lr = tf.maximum(lr, 1e-4)
    ...
    for c in range(len(pc_classes)):    # attack for each class     len(pc_classes)
        ...
        for b in range(n_batches):   # generating adv_examples by # of batchs    # n_batches
            ...
            feed_dicts = {inputs: p_target_pc, npts: npts_batch, gt: target_pc_batch} 
            ...
            for iteration in range(args.num_iterations):    # args.num_iterations
                feed_dicts = {inputs: p_source_pc, npts: npts_batch, gt: target_pc_batch}
                _ = sess.run(attack_op, feed_dict=feed_dicts)
                adv_point_cloud = sess.run(adv_pc, feed_dict=feed_dicts)

                delta = pointwise_project_constraint(adv_point_cloud - p_source_pc, args.radius, p_source_pc)
    ...
    df.to_csv(os.path.join(save_dir, 'statistic_table_geometry_pointwise_radius%s.csv' % str(args.radius)), encoding="utf-8-sig", mode="a", header=True, index=False)
