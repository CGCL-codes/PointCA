...
from utils.pcn_tf_util import mlp, mlp_conv, point_maxpool, point_unpool, chamfer, \
...
class Model:
    def __init__(self, inputs, npts, gt, alpha, **kwargs):
        ...
        self.encoder_point_features, self.encoder_local_features, self.features = self.create_encoder(inputs, npts)
        self.coarse, self.fine, self.decoderfeat = self.create_decoder(self.features)
    ...
    def create_encoder(self, inputs, npts):
        with tf.variable_scope('encoder_0', reuse=tf.AUTO_REUSE):
            features_point = mlp_conv(inputs, [128, 256])
            features_global = point_unpool(point_maxpool(features_point, npts, keepdims=True), npts)
            features = tf.concat([features_point, features_global], axis=2)
        with tf.variable_scope('encoder_1', reuse=tf.AUTO_REUSE):
            features = mlp_conv(features, [512, 1024])
            features_vector = point_maxpool(features, npts)
        return  features_point, point_maxpool(features_point, npts, keepdims=True), features_vector
